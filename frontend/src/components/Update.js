...
const Update = ({
  ...
  const handleDefaultThumbnail = () => {
    setDefaultThumbnail(true);
  };

  async function handleSave() {
    var formData = new FormData();
    if (thumbnail) formData.append(defaultThumbnail);
    ...
    formData.append("id", id); // id file 추가
    const results = await fetch(`https://my-wiki.p-e.kr/api/board/update`, {
      ...
      <div className="thumbnail-container">
        ...
        {thumbnail && (
          <div>
            ...
            <button className="remove-button" onClick={handleDefaultThumbnail}>
              썸네일 제거
            </button>
          </div>
        )}
